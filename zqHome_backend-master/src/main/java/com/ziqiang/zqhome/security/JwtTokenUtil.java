...
public class JwtTokenUtil {
    ...
    public static Claims parseJWT(String jsonWebToken, String base64Security) {
        try {
            ...
            throw new ZqHomeException(ResultEnum.PERMISSION_TOKEN_EXPIRED);
        } catch (Exception e){
            log.error("===== token解析异常 =====", e);
            throw new ZqHomeException(ResultEnum.PERMISSION_TOKEN_INVALID);
        }
    }
    /**
     * 构建jwt
...
//                System.out.println(exp);
            ...
            log.error("签名失败", e);
            throw new ZqHomeException(ResultEnum.PERMISSION_SIGNATURE_ERROR);
        }
    }
    /**
     * 从token中获取用户名
